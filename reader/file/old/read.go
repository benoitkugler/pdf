...
func ReadFile(inFile string, conf *Configuration) (*Context, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("can't open %s", inFile)
	}
	defer f.Close()
...
func parseXRefStream(rd io.Reader, offset *int64, ctx *Context) (prevOffset *int64, err error) {
	...
	if ctx.XRefTable.Exists(*objectNumber) {
		...
		ctx.XRefStreams[*objectNumber] = true
	}
...
func parseHybridXRefStream(offset *int64, ctx *Context) error {
	...
	rd, err := newPositionedReader(ctx.rs, offset)
	if err != nil {
		return err
	}
...
func parseTrailerDict(trailerDict Dict, ctx *Context) (*int64, error) {
	...
	if offsetXRefStream == nil {
		...
		if !ctxer15 && xRefTable.Version() >= V14 && !ctx.Hybrid {
...
func buildXRefTableStartingAt(ctx *Context, offset int64) (err error) {
	rs := ctx.rs
	...
	for offset != 0 {
		...
		if offs[offset] {
			offset, err = offsetLastXRefSection(ctx, ctx.FileSize-offset)
			if err != nil {
				return err
			}
			if offs[offset] {
...
func loadEncodedStreamContent(ctx *Context, sd *StreamDict) ([]byte, error) {
	...
	rd, err := newPositionedReader(ctx.rs, &newOffset)
	if err != nil {
		return nil, err
	}
...
func loadStreamDict(ctx *Context, sd *StreamDict, objNr, genNr int) error {
	...
	ctx.BinaryTotalSize += *sd.StreamLength
...
func updateBinaryTotalSize(ctx *Context, o Object) {
	...
	case StreamDict:
		ctx.BinaryTotalSize += *o.StreamLength
