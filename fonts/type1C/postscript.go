// Compact Font Format, adapted from golang.org/x/image/font/sfnt
//
...
func (p *cffParser) parse() (ret glyphData, err error) {
	// Parse the header.
	buf, err := p.read(4)
	if err != nil {
	...
	if err := p.parseIndexLocations(p.locBuf[:2], offSize); err != nil {
		return glyphData{}, err
	}
	...
	buf, err = p.read(int(p.locBuf[1] - p.locBuf[0]))
	if err != nil {
		return glyphData{}, err
	}
	...
	if count != 0 {
		...
		if err := p.skip(int(count * offSize)); err != nil {
			return glyphData{}, err
		}
		buf, err := p.read(int(offSize))
		if err != nil {
			return glyphData{}, err
		}
		loc := bigEndian(buf) - 1
	...
	if err := p.seek(p.psi.topDict.charStringsOffset); err != nil {
		return glyphData{}, err
	}
	...
	// Parse the charset INDEX, whose location was found in the Top DICT.
	// Predefined charset may have offset of 0 to 2 // Table 22
	var charset []uint16
	switch charsetOffset := p.psi.topDict.charsetOffset; charsetOffset {
	case 0: // ISOAdobe
	...
	default: // custom
		...
		fmt.Println("custom format", buf[0])
		switch buf[0] { // format
		case 0:
			...
			for i := 1; i < int(numGlyphs); i++ {
				charset[i] = be.Uint16(buf[2*i-2:])
			}
		case 1:
			for i := 1; i < int(numGlyphs); {
				buf, err := p.read(3)
				if err != nil {
					return glyphData{}, err
				}
				first, nLeft := be.Uint16(buf), uint16(buf[2])
				for j := uint16(0); j <= nLeft && i < int(numGlyphs); j++ {
					charset[i+int(j)] = first + j
				}
				i += int(nLeft) + 1
			}
		case 2:
...
loop:
	for len(p.instructions) > 0 {
		...
		for escaped, ops := false, psOperators[ctx][0]; ; {
			...
			if escaped {
				return fmt.Errorf("unrecognized CFF 2-byte operator (12 %d)", b)
			} else {
				return fmt.Errorf("unrecognized CFF 1-byte operator (%d)", b)
			}
		}
	}
	return nil
...
var psOperators = [...][2][]psOperator{
	...
	psContextTopDict: {{
		...
		13: {+1, "UniqueID", nil},
		14: {-1, "XUID", nil},
		15: {+1, "charset", func(p *psInterpreter) error {
			p.topDict.charsetOffset = p.argStack.a[p.argStack.top-1]
			return nil
		}},
		16: {+1, "Encoding", func(p *psInterpreter) error {
			p.topDict.encodingOffset = p.argStack.a[p.argStack.top-1]
			return nil
		}},
